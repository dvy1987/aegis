...
export async function resolveShowcaseManifest(): Promise<ShowcaseManifestLoad> {
  const base = getApiBase();
  const legacyWarning =
    "Showcase API is on the legacy 8/80 cohort. The grid uses the bundled 3/1 + 5/2 manifest. Redeploy the drafting service (or point NEXT_PUBLIC_AEGIS_API at a current backend) before starting a run.";
