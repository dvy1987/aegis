...
Date: 2026-06-10
Status: feature built (backend Phases 1–7 + `/appeal` frontend chat step); **tests written but NOT run** (only `py_compile` syntax checks and a frontend `tsc --noEmit` were executed). Your job: run the suites below, fix small fallout, and report.

Design doc: `docs/specs/2026-06-10-question-agent-design.md`
Key invariants: appeal Q&A = traced, NOT graded; showcase Q&A = graded; INV-QA = student/question agent never sees teacher `clinical_context` in showcase; geo playbook work is DEFERRED (owned by another agent — do not touch `geo_playbook.py`).
...
Extra test fallout to expect from Round 2: pipeline/workflow tests asserting the exact node/edge list or `StudentWorkflowState` fields; tests asserting `AppealPackage` has no extra fields; `test_question_judge.py` was rewritten for the teacher-driven Part B; offline `run_evaluated_case(..., run_question_agent=True)` now logs Gemini-fallback warnings (stub fallback) — harmless.
...
- `backend/app/learning/coordinator.py` — `question_agent_system_prompt` registered as an evolvable component (eligible ids, seed candidate, global slice filter).
- `backend/app/learning/reflection_client.py` — playbook reflections get an APPEND-FIRST rule + question-judge recs (ride the laundered `question_agent` improvement notes; geo explicitly excluded).
...
From `backend/`:
```
python -m pytest tests/unit/aegis_v1/test_question_agent.py tests/unit/aegis_v1/test_question_firewall.py tests/unit/evals/test_question_judge.py -v
```
- `test_question_agent.py` (11 tests) passed earlier in the session; the other two files have never been executed.
- `test_question_firewall.py` holds the INV-QA build-breaking tests + session flow tests.

### 2. Backend — regression on touched areas
```
python -m pytest tests/unit/evals/test_evaluated_run.py tests/unit/evals/test_part_a_judge_panel.py tests/unit/evals/test_measurement_run.py tests/unit/evals/test_firewall.py tests/unit/evals/test_judge_workflow_phase3.py -v
python -m pytest tests/unit/aegis_v1/test_appeal_route.py tests/unit/aegis_v1/test_appeal_orchestrator.py tests/unit/aegis_v1/test_appeal_best_of_five.py -v
python -m pytest tests/unit/learning/test_coordinator.py tests/unit/learning/test_integration.py tests/unit/learning/test_efficacy_harness.py -v
```
Likely fallout to expect and fix:
...
- `tests/unit/aegis_v1/test_showcase_rollback.py::test_promotion_stack_snapshots_and_restores_changed_files` fails standalone (playbook version `cigna_mednec_v3` vs expected `day_zero`). Pre-existing.
- An earlier full-suite run (mid-build, before panel/judge wiring was complete) hit ~59 COLLECTION errors across `aegis_swarm`/`evals`/`learning`. Cause was never diagnosed (the run was aborted to save credits). If collection errors reappear, run `python -m pytest tests/ --collect-only -q 2>&1 | Select-String error -Context 3` first — one bad import in a shared module cascades to everything. Verify imports of the new modules (`question_session`, `question_judge`) resolve before assuming wider breakage.
...
- Geo playbook integration — owned by another agent; revisit when both land.
- Promotion → activation for the question prompt: promoted `question_agent_system_prompt` versions land in the learning store; runtime activation still reads `AEGIS_QUESTION_AGENT_PROMPT_VERSION` env or `prompts/active_question_agent_prompt.txt`. Wiring the promotion stack to write that file mirrors how the drafter prompt is activated.
