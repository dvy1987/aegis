# Rolling Demo Capture Checklist — Heuristics
...
- **Left:** The Heuristics app (your Next.js frontend — currently `http://localhost:3000`, later a Cloud Run URL)
...
1. Browser left tab: `http://localhost:3000` (Heuristics app — the workbench page, once built)
...
|---|---|---|---|
| 1. Load a case | Heuristics app (left) | Pick a Cigna medical-necessity case from the selector | "Loading a Cigna denial case" |
| 2. Hit "Draft Appeal" | Heuristics app (left) | The agent processes the case | (wait for output) |
| 3. Show the v1 appeal | Heuristics app (left) | A generic, weak appeal letter — no insurer-specific language, no plan citations | "This is v1. Generic. No Cigna-specific tactics." |
| 4. Show eval scores | Heuristics app (left) | Low composite score (expected ~0.40-0.55) | "Composite score is low." |
| 5. Show simulator result | Heuristics app (left) | Simulator says DENY | "The simulator says this would be denied." |
...
| 3. Switch to Heuristics | Heuristics app (left) | Load same case, show v2 appeal output | "The v2 appeal cites plan language." |
| 4. Show simulator flip | Heuristics app (left) | Simulator now says APPROVE (or closer to it) | "The simulator outcome improved." |
...
| Step | Where | What you see | What to say |
|---|---|---|---|
| 1. v1 appeal | Heuristics app (left) | Load hero case, show v1 weak appeal | "This is where we started." |
| 2. v1 scores + simulator | Heuristics app (left) | Low composite, simulator DENY | "Weak score. Simulator denies." |
...
| 4. Approved patch | Heuristics app (left) | Show the prompt/playbook change that was approved | "We approved this patch." |
| 5. v3 appeal | Heuristics app (left) | Same case, now with strong appeal citing plan language | "After learning, the appeal is specific." |
| 6. v3 scores + simulator | Heuristics app (left) | Higher composite (~0.75), simulator APPROVE | "Score jumped. Simulator approves." |
| 7. Benchmark chart | Heuristics app (left) or Phoenix (right) | v1 vs v3 across all held-out cases | "Improvement is consistent." |
...
**Before you record, open these:**
1. Heuristics app (left) — should now show agent status/progress
...
| Step | Where | What you see | What to say |
|---|---|---|---|
| 1. Load case | Heuristics app (left) | Case goes into the swarm | "Now it's a team of agents." |
| 2. Show agent activity | Heuristics app (left) | Multiple agents working in parallel (Triage, Researchers, Strategist, Drafter) | "9 agents working in parallel." |
...
**Before you record, open these:**
1. Heuristics app (left) — v1→v_current comparison chart
...
| 4. Benchmark chart | Heuristics app (left) | The full benchmark results chart | "Consistent improvement across all cases." |
| 5. Safety stable | Heuristics app (left) or Phoenix (right) | Safety gate PASS across all versions | "And safety never regressed." |
...
| Step | Where | What you see | What to say |
|---|---|---|---|
| 1. Run with Phoenix ON | Heuristics app (left) | Normal run, good score, simulator APPROVE | "With Phoenix MCP enabled, the agent works well." |
| 2. Toggle Phoenix OFF | Heuristics app (left) or backend env | Set `PHOENIX_MCP_ENABLED=false`, re-run same case | "Now I disable Phoenix." |
| 3. Show collapse | Heuristics app (left) | Same case, but now generic appeal, low score, simulator DENY | "Without Phoenix, the agent forgets everything it learned." |
...
## Quick Reference: What the Heuristics App Needs to Show

For the recordings to work, the Heuristics frontend must have these surfaces built by the relevant day:
...
**Yes, the UX shows the simulator outcome.** Per PRD FR8 and FR10, the Heuristics frontend must display the simulator's APPROVE or DENY verdict alongside the eval scores. This is a core demo element:
