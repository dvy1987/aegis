...
**Rationale.** PM requested per-stage critics instead of one critic-in-loop at the end — the latter matches the AlphaEval mega-prompt anti-pattern. Per-stage independent critics catch issues at the artifact that produced them and prevent dimension bleed. PM said "operate as if Gumloop/ChatGPT/Perplexity curators do not exist" → swarm must be self-sufficient → absorbed Gumloop's 7 dimensions and added 4 gap-coverage critics. Weighted random sampling (PM choice) over force-cover for the diversity matrix. Full structured provenance for auditability.
