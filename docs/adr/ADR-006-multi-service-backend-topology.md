...
**Context:**
For the final 3-minute hackathon demo, we need to clearly show the contrast between the failing single-agent (Heuristics v1) and the successful self-improving swarm (Part B). Additionally, the offline case generation pipeline currently requires raw API keys for Claude and Gemini, creating a security and configuration hurdle.
...
2. **Logical Backend Split:** We will keep a single Python backend repository but launch it as two isolated logical services (OS processes):
   - **Heuristics v1 API:** Runs on Port 8001, uses `PHOENIX_PROJECT_NAME=default` (pinned in `main_v1.py`).
   - **Heuristics Swarm API:** Runs on Port 8002, uses `PHOENIX_PROJECT_NAME=aegis-swarm` (pinned in `main_swarm.py` / `Dockerfile.swarm`).

   > **Amendment (2026-06-07):** Original ADR named Phoenix projects `aegis-baseline` and `aegis-swarm`. PM confirmed deployed names are **`default`** (v1) and **`aegis-swarm`** (swarm). There is no Phoenix project called `aegis-swarm`. See [decision-log.md §2026-06-07 Phoenix project split](../memory/decision-log.md).
...
**Consequences:**
- ✓ Complete isolation of Phoenix traces into `default` (v1) and `aegis-swarm` (swarm) — perfect for the demo arc.
