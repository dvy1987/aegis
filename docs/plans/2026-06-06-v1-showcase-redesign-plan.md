...
- **v1 writes to Phoenix project `default`**, not `aegis-swarm`. Swarm writes to `aegis-swarm`. They use different recorder classes (`OtelPhoenixRecorder` vs `OtelSwarmTraceRecorder`); they cannot collide. See [decision-log.md §2026-06-07 Phoenix project split](../memory/decision-log.md).
