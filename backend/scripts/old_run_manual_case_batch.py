#!/usr/bin/env python3
"""LEGACY — pre–realism-pass batch generator (old Dear Member letters).

Prefer: ``run_manual_case_batch.py`` or ``uv run python -m app.case_generator.cli``.
"""
...
from app.case_generator.manual_batches.old_manual_producer import (  # noqa: E402
    run_manual_pipeline,
)
...
def main() -> int:
    ...
    neighbours: list[str] = []
    written = 0
    print(f"run_id={run_id} batch={args.batch} cases={len(items)} [LEGACY]")
