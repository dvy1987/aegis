...
def apply_safe_fixes(case: dict[str, Any]) -> dict[str, Any]:
    ...
    # Normalize demographics to schema-safe codes.
    prof = dict(case.get("patient_profile") or {})
    gender_raw = str(prof.get("gender") or "").strip()
    gender_l = gender_raw.lower()
    if gender_raw not in {"F", "M", "nonbinary"}:
        if gender_l in {"female", "woman", "f"}:
            prof["gender"] = "F"
        elif gender_l in {"male", "man", "m"}:
            prof["gender"] = "M"
        elif gender_l in {"nonbinary", "non-binary", "nb", "genderqueer"}:
            prof["gender"] = "nonbinary"
        else:
            prof["gender"] = "nonbinary"
    age = prof.get("age")
    if not isinstance(age, int) or age < 18 or age > 89:
        # Use matrix cell age band if present; otherwise choose a middle adult default.
        age_band = (
            (case.get("synthetic_provenance") or {}).get("matrix_cell") or {}
        ).get("patient_age_band")
        band_map = {
            "18-25": 22,
            "26-40": 33,
            "41-55": 48,
        ...
        prof["age"] = band_map.get(str(age_band), 45)
    ...
    if "circular_medical_necessity" in pattern_ids:
        # Ensure the canonical circular sentence exists somewhere in the decision explanation.
        circ = "not medically necessary because it does not meet the plan's medical necessity criteria"
        if circ not in denial.lower():
            denial = (
                denial.strip()
