...
def demographic_validator(case: dict[str, Any]) -> PassFailCriticOutput:
    ...
    ok = isinstance(age, int) and 18 <= age <= 89 and gender in {"F", "M", "nonbinary"}
    verdict = "PASS" if ok else "FAIL"
    improvement = None
    if verdict == "FAIL":
        improvement = (
            "In patient_profile, age or gender is invalid. Fix: set age to an integer 1–109 and gender to one of "
            "['M','F','nonbinary'] and ensure clinical_context matches."
        )
    return {
...
def flaw_injection_verifier(case: dict[str, Any]) -> FlawInjectionVerifierOutput:
    ...
    for pid in pattern_ids:
        ...
        if pid == "missing_cost_liability":
            ...
            # Detect the canonical circular phrasing in a tolerant way:
            # - either the joined regex match
            # - or both key fragments present somewhere in the denial
            has_joined = bool(re.search(r"not medically necessary.*meet medical necessity", denial_l, re.S))
            has_fragments = ("not medically necessary" in denial_l) and ("medical necessity criteria" in denial_l)
            status = "PRESENT" if (has_joined or has_fragments) else "ABSENT"
            evidence = (
                "Matched circular phrasing (regex or fragments present)."
                if status == "PRESENT"
                else "No 'not medically necessary' + 'medical necessity criteria' match."
            )
        else:
